...
func _init() -> void:
    ScaffoldUtils.print("ScaffoldLevelConfig._init")
    
    _clear_old_version_level_state()
    
    _sanitize_level_configs()
    
    SaveState.set_level_is_unlocked(get_level_ids().front(), true)
...
func _clear_old_version_level_state() -> void:
    if ScaffoldConfig.score_version != SaveState.get_score_version():
        SaveState.set_score_version(ScaffoldConfig.score_version)
