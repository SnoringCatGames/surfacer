...
static func calculate_jump_land_positions_for_surface_pair( \
    ...
    if jump_surface_has_only_one_point or land_surface_has_only_one_point:
        ...
        ScaffoldUtils.error("Single-point surfaces are not yet supported in" + \
    ...
    match jump_surface.side:
        SurfaceSide.FLOOR:
            match land_surface.side:
                            ...
                            if is_enough_left_side_distance_to_not_backtrack_horizontally:
                                ...
                                if is_jump_surface_lower:
                                    ...
                                    var jump_basis: Vector2 = \
                                            Geometry.project_point_onto_surface_with_offset( \
                ...
                SurfaceSide.LEFT_WALL, SurfaceSide.RIGHT_WALL:
                    ...
                    if can_jump_in_front_of_wall:
                        ...
                        if !do_surfaces_overlap_horizontally:
                            ...
                            land_basis = Geometry.project_point_onto_surface_with_offset( \
                            ...
                            var goal_x := \
                                    ...
                                    land_surface_bottom_end.x - \
                                    player_half_width_horizontal_offset
                            jump_basis = Geometry.project_point_onto_surface_with_offset( \
                                    Vector2(goal_x, INF), \
                                    jump_surface, \
                                    movement_params.collider_half_width_height)
                ...
                _:
                    ScaffoldUtils.error("Unknown land surface side (jump from floor)")
            ...
            match land_surface.side:
                SurfaceSide.FLOOR:
                    ...
                    if is_wall_fully_higher_than_floor:
                        ...
                        var jump_basis: Vector2 = Geometry.project_point_onto_surface_with_offset( \
                ...
                SurfaceSide.LEFT_WALL, SurfaceSide.RIGHT_WALL:
                    ...
                    if jump_surface.side == land_surface.side:
                        ...
                        jump_basis = Geometry.get_closest_point_on_polyline_to_polyline( \
                                jump_surface.vertices, \
                                land_surface.vertices)
                        jump_basis = Geometry.offset_point_from_surface( \
                                jump_basis, \
                                jump_surface, \
                                movement_params.collider_half_width_height)
                        land_basis = Geometry.get_closest_point_on_polyline_to_point( \
                                land_basis, \
                                land_surface.vertices)
                        land_basis = Geometry.offset_point_from_surface( \
                                land_basis, \
                                land_surface, \
                                movement_params.collider_half_width_height)
                ...
                SurfaceSide.CEILING:
                    ...
                    ScaffoldUtils.error("calculate_jump_land_positions_for_surface_pair ceiling cases " + \
                            "not implemented yet")
                    
                _:
                    ScaffoldUtils.error("Unknown land surface side (jump from wall)")
        ...
        SurfaceSide.CEILING:
            match land_surface.side:
                ...
                _:
                    ScaffoldUtils.error("Unknown land surface side (jump from ceiling)")
            
        _:
            ScaffoldUtils.error("Unknown jump surface side")
...
static func calculate_land_positions_on_surface( \
    ...
    match land_surface.side:
        ...
        _:
            ScaffoldUtils.error("Unknown land surface side")
            return []
