...
func _enter_tree() -> void:
    ScaffoldConfig.canvas_layers.create_layer( \
            "ruler", \
            ScaffoldConfig.canvas_layers.layers.annotation.layer + 50, \
            Node.PAUSE_MODE_STOP)
    
    element_annotator = ElementAnnotator.new()
    ScaffoldConfig.canvas_layers.layers.annotation.add_child(element_annotator)
