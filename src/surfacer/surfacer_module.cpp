...
void Surfacer::register_gdextension_types(ModuleInitializationLevel p_level) {
	...
	// Surfacer's registrar; a downstream consumer that also calls
	// Surfacer::register_gdextension_types directly would otherwise
	// re-enter.
	...
	// registered Surfacer (e.g., a stray standalone surfacer.gdextension
	...
	// See bootstrapper HANDOVER Phase 2.1 finding (a).
	if (ClassDB::class_exists("Surfacer")) {
		WARN_PRINT(
				"Surfacer GDExtension classes are already registered "
				"by another loaded extension; skipping duplicate "
				"registration. Only one extension (surf_scaf, the "
				"canonical bundle) should register Surfacer classes.");
		return;
	}
