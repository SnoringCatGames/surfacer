...
func start(metric: int) -> void:
    ...
    assert(!_start_times_msec.has(metric) or _start_times_msec[metric] == -1)
    _start_times_msec[metric] = start_time

# Returns the elapsed time in milliseconds.
func stop(metric: int) -> float:
    var stop_time := OS.get_ticks_usec()
    var start_time: int = _start_times_msec[metric]
    assert(start_time >= 0)
    var elapsed_time := stop_time - start_time
    _start_times_msec[metric] = -1
