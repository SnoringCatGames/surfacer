...
function solve_for_movement_duration(s_0, s, v_0, a, returns_lower_result) {
    ...
    if (t1 < 0) {
        ...
        return t1;
    } else if (returns_lower_result) {
                return Math.min(t1, t2);
        } else {
                return Math.max(t1, t2);
        }
}
function calc_duration_for_tests(
        is_position_before_instruction_end,
                is_position_before_peak) {
    var s_0 = is_position_before_instruction_end ? 0 : -100;
	var v_0 = is_position_before_instruction_end ? -1000 : -500;
	var a = is_position_before_instruction_end ? 900 : 5000;
	var s = is_position_before_instruction_end ? -100.0 : -110.0;
    console.log(solve_for_movement_duration(s_0, s, v_0, a, is_position_before_peak));
}
calc_duration_for_tests();

// FIXME: REMOVE
function calculate_time_to_release_acceleration(time_start, time_step_end, position_start,
        position_end, velocity_start, acceleration_start,
...
function calculate_time_for_tests() {
	var time_start = 0;
    var time_step_end = 0.9;
	var position_start = 0;
    var position_end = 180;
	var velocity_start = 0;
	var acceleration_start = 1500;
    var post_release_backward_acceleration = 0;
	var returns_lower_result = true;
    var expects_only_one_positive_result = false;
    var result = calculate_time_to_release_acceleration(time_start, time_step_end,
	    position_start, position_end, velocity_start, acceleration_start,
        post_release_backward_acceleration, returns_lower_result,
        expects_only_one_positive_result);
	console.log(result);
}
calculate_time_for_tests();
```
